 ...
 */
package org.ethereumphone.testwalletbridgeapp.bridge;
	...
	public synchronized void push(WebSocket conn, WalletConnectMessage msg) {
		...
		while (subs.hasNext()) {
			...
			if (msg.shouldDeliverTo(subscriber)) {
				try {
					System.out.println("Deliviering " + msg + " from " + conn + " to " + subscriber.getRemoteSocketAddress());
					subscriber.send(msg.getJson());
//					subscriber.sendPing();
				} catch (WebsocketNotConnectedException e) {
					System.out.println("Lost subscriber: " + subscriber.getRemoteSocketAddress());
					subs.remove();
				}
			}
		}
	...
	public synchronized void sub(WebSocket conn, WalletConnectMessage sub) {
		this.touch();
		try {
			System.out.println(conn.getRemoteSocketAddress() + " wants to know about " + sub.topic);
			for (WalletConnectMessage msg : messages) {
				if (msg.shouldDeliverTo(conn)) {
					System.out.println("Deliviering " + msg + " to " + conn.getRemoteSocketAddress());
...
//			conn.sendPing();
			...
			System.out.println("Lost subscriber: " + conn.getRemoteSocketAddress());
		}
	}
